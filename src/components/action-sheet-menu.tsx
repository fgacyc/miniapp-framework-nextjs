...
const ActionSheetButton: React.FC<ActionSheetButtonProps> = ({ title, icon, onClick }) => {
    return (
        <div className={"flex flex-col items-center"}>
            <button
                className={"flex flex-col items-center justify-center bg-gray-100 w-12 h-12 rounded-lg cursor-pointer"}
                onClick={onClick}
            >
                {icon}
            </button>
        ...
        <div className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  bg-[#ffffff] 
            ...
            <div className={"h-full overflow-auto p-2"}>
                ...
                <div className={"my-1"}>
                    ...
                    <textarea defaultValue={token ? "http://localhost:3000/?token=" + token + "&language=" + lang : ""}
                              rows={textAreaRow}
                              className={`w-full border border-black`}/>
                </div>
                <div>
